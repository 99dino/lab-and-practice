...
process find_min(process proc_arr[],int size){
	if(size<1){
...
void min_heapify(process proc_arr[],int i,int n){
    int max=i;
    int left=2*i;
    int right=2*i+1;
    ...
    if(max!=i){
        ...
        min_heapify(proc_arr,max,n);
    }
}

process extract_min(process proc_arr[],int* size){
	if(*size<1){
		printf("Invalid operation.\n");
        return proc_arr[0];
	}
	...
	(*size)--;
	min_heapify(proc_arr,1,*size);
	return temp;
}

void decrease_key(process proc_arr[],int i,int size,int key){
...
void insert(process proc_arr[],int* size,process newproc){
	...
	proc_arr[*size].key=INT_MAX;
	decrease_key(proc_arr,*size,*size,newproc.key);
}

void build_min_heap(process proc_arr[],int n){
    for(int i=n/2;i>0;i--){
        min_heapify(proc_arr,i,n);
    }
}
...
void priority_queue(process array[],int n){
	...
	do{
		...
		switch(a){
			case 1:
				printf("-->%d\n",(find_min(array,n)).key);
                break;
			case 2:
				printf("-->%d\n",(extract_min(array,&n)).key);
                break;
			case 3:
